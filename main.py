...
class MainWindow(QMainWindow):
        ...
        try:
            ...
            for candidate in ["assets/gym_logo.ico", "assets/gym_icon.ico", "assets/icon.png"]:
                ...
                if os.path.exists(icon_path):
                    ...
                    break
            self.resize(1600, 1000)  # Tamaño inicial optimizado para pantallas grandes
            self.setMinimumSize(1400, 900)  # Tamaño mínimo aumentado para ventana maximizada
            ...
            # Inicializar gestor de sincronización offline
            try:
                from offline_sync_manager import OfflineSyncManager
                self.offline_sync_manager = OfflineSyncManager()
                # Adjuntar managers para ejecución
                self.offline_sync_manager.attach_managers(self.db_manager, self.whatsapp_manager)
                # Exponer referencias para que DB y WhatsApp lo usen
                setattr(self.db_manager, 'offline_sync_manager', self.offline_sync_manager)
                if self.whatsapp_manager:
                    setattr(self.whatsapp_manager, 'offline_sync_manager', self.offline_sync_manager)
                # Arrancar worker en background
                self.offline_sync_manager.start_background_worker()
                logging.info("OfflineSyncManager inicializado y en ejecución")
            except Exception as e:
                logging.warning(f"No se pudo inicializar OfflineSyncManager: {e}")
            ...
            # Aplicar visibilidad de pestañas principales por rol desde configuración persistida
            ...
            # Aplicamos la hoja de estilos al final
            try:
                self.apply_complete_styling()
            except Exception as e:
                logging.error(f"Error en apply_complete_styling: {e}", exc_info=True)
    ...
    def setup_tabs(self):
        ...
        try:
            self.tabs['reportes'] = ReportsTabWidget(self.db_manager, self.payment_manager, self.export_manager)
            # Inyectar referencia a la ventana principal para que ReportsTabWidget
            # pueda obtener correctamente el usuario actual sin depender de window()
        ...
        # Reorganización por frecuencia de uso: usuarios, pagos, reportes primero
        self.tab_indices = {}
    ...
    def update_connectivity_indicator(self):
        """Actualiza el indicador de conectividad usando OfflineSyncManager si está disponible."""
        try:
            snapshot = None
            if hasattr(self, 'offline_sync_manager') and self.offline_sync_manager:
                try:
                    snapshot = self.offline_sync_manager.get_connectivity_snapshot()
                except Exception:
                    snapshot = None

            # Fallback mínimo si no hay offline manager
            if snapshot is None:
                internet_ok = True
                db_ok = True
                whatsapp_ok = True
            ...
            db_ok = snapshot.get('db_ok', False)
            whatsapp_ok = snapshot.get('whatsapp_ok', False)
            pending_ops = snapshot.get('pending_ops', 0)
            pending_breakdown = snapshot.get('pending_breakdown', {}) or {}
            actionable_whatsapp = pending_breakdown.get('actionable_whatsapp', 0)
            total_whatsapp = pending_breakdown.get('whatsapp', 0)
            scheduled_total = pending_breakdown.get('scheduled', 0)
            scheduled_db = pending_breakdown.get('scheduled_db', 0)
            scheduled_wa = pending_breakdown.get('scheduled_whatsapp', 0)
            ...
            tooltip = (
                ...
                f"Cola offline: {pending_ops} pendientes\n"
    ...
    def setup_notification_timer(self):
        ...
        # Ejecutar procesamiento inicial poco después de iniciar la app (10 segundos)
        QTimer.singleShot(10000, self.process_whatsapp_automation)
    ...
    def process_whatsapp_automation(self):
        ...
        try:
            ...
            logging.info("Iniciando procesamiento automático de pagos y estados")
            ...
            total_actividad = recordatorios_enviados + morosos_procesados + resultados_vencimientos.get('usuarios_desactivados', 0)
            ...
            logging.error(f"Error en procesamiento automático de WhatsApp: {e}")
            alert_manager.generate_alert(
                AlertLevel.ERROR,
                AlertCategory.SYSTEM,
    ...
    def tab_changed(self, index):
        ...
        if widget == self.tabs.get('rutinas'):
            ...
            self.tabs['reportes'].actualizar_reportes()
    ...
    def update_monthly_hours(self):
        """Actualiza el contador de horas mensuales en tiempo real"""
        try:
            ...
            if self.user_role == 'profesor' and self.logged_in_user:
                ...
                if profesor_id:
                    ...
                    if resultado.get('success'):
                        # El backend devuelve 'total_minutos' y también 'horas_display'/'minutos_display'
            ...
            logging.error(f"Error actualizando horas mensuales: {e}")
            self.monthly_hours_label.setText("⏰ Horas del mes: Error")
...
def main():
    ...
    # Iniciar servidor web (dashboard) en segundo plano con la misma instancia de DB
    try:
        from webapp.server import start_web_server, start_public_tunnel, set_public_tunnel_reconnect_callback
        ...
        try:
            ...
            port = 8000
        start_web_server(db_manager_for_login, host=host, port=port)
        # Determinar IP local para mostrar la URL accesible desde otros dispositivos
        try:
            import socket
            local_ip = "127.0.0.1"
            try:
            ...
            logging.info(
                f"Servidor web de dashboard iniciado. Acceso local: http://127.0.0.1:{port}/ | Acceso en red: http://{local_ip}:{port}/"
            )
            ...
            logging.info("Servidor web de dashboard iniciado en puerto %s", port)
            logging.debug(f"No se pudo determinar IP local: {e}")
    except Exception as e:
        logging.warning(f"No se pudo iniciar el servidor web: {e}")
    ...
    try:
        if login_dialog.exec():
            ...
            # Reforzar el icono en la barra de tareas y ventana principal tras el login
            try:
            ...
            # Integrar monitor de salud de redes con reinicios automáticos
            try:
                # Exponer parámetros a la ventana para botones/acciones
                try:
                ...
                # Callbacks de reinicio
                def _restart_server_cb():
                    try:
                        start_web_server(db_manager_for_login, host=host, port=port)
                    except Exception:
                        pass
            ...
            try:
                # En caso de que el import anterior falle en otro contexto, mantenemos un import local seguro
                from webapp.server import set_public_tunnel_reconnect_callback as _set_cb
            ...
            try:
                ...
                try:
                    ...
                    public_url = None
                try:
                    toast = ServerStatusToast(window, local_url=local_url, public_url=public_url)
                    toast.show_toast()
                except Exception:
                    pass
            except Exception:
