        ...
        try:
            with self.db_manager.get_connection_context() as conn:
                ...
                if result and result['count'] > 0:
                    cursor.execute("SELECT id, nombre FROM usuarios WHERE activo = true ORDER BY nombre")
                    
                    for row in cursor.fetchall():
                        # Usar nombres de columna en lugar de índices con RealDictCursor
                        self.user_filter.addItem(row['nombre'], row['id'])  # nombre, id
                else:
                    logging.warning("No hay usuarios activos en la base de datos")
                    
        except Exception as e:
            logging.error(f"Error cargando usuarios para filtro: {e}")
            # Agregar un usuario por defecto para evitar errores
            self.user_filter.addItem("Sin usuarios disponibles", None)
