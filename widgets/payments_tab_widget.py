...
class PaymentsTabWidget(QWidget):
    ...
    def create_user_header(self):
        ...
        self.payment_status_label = QLabel("Seleccione un usuario...")
    ...
    def load_initial_data(self):
        ...
        try:
            ...
            self.all_users = self.db_manager.obtener_todos_usuarios()
            
            self.user_combobox.blockSignals(True)
            self.user_combobox.clear()
            self.user_combobox.addItem("", userData=None)
            
            index_to_select = 0
            for i, user in enumerate(self.all_users, 1):
                self.user_combobox.addItem(f"{user.id} - {user.nombre}", userData=user)
                if user.id == current_selection_id:
                    index_to_select = i
    ...
    def load_payment_methods(self):
        ...
        try:
            methods = self.payment_manager.obtener_metodos_pago()
            ...
            QMessageBox.warning(self, "Advertencia", f"Error al cargar métodos de pago: {str(e)}")
    
    def load_payment_concepts(self):
        """Carga los conceptos de pago en la tabla moderna"""
        try:
            concepts = self.payment_manager.obtener_conceptos_pago()
            
            # Actualizar modelo de tabla
            self.concepts_model.update_data(concepts)
            
            # Inicializar precios base correctamente
            for concept in concepts:
                if hasattr(concept, 'precio_base'):
                    # No sobreescribir precio base de conceptos variables.
                    # El precio de "Cuota Mensual" debe derivarse del tipo de cuota del usuario seleccionado.
                    
                    # Inicializar precio en el modelo
    ...
    def update_payment_history(self):
        ...
        try:
            history = self.payment_manager.obtener_historial_pagos(self.selected_user.id)
            self.history_model.update_data(history)
            
            # Habilitar botones de exportación solo si hay datos
            has_data = len(history) > 0
            self.pdf_button.setEnabled(has_data)
            self.excel_button.setEnabled(has_data)

            # Mostrar indicador coherente cuando no hay pagos
            if not has_data:
                self.payment_status_label.setText("<b>📭 SIN PAGOS REGISTRADOS</b>")
                self.payment_status_label.setProperty("paymentStatus", "no_payments")
                self.payment_status_label.style().unpolish(self.payment_status_label)
            ...
            logging.exception(f"Error al actualizar historial de pagos: {e}")
            self.history_model.update_data([])
    ...
    def load_default_payment_amount(self):
        """Carga conceptos por defecto"""
        try:
            tipos_activos = self.db_manager.obtener_tipos_cuota_activos()
            if tipos_activos:
            ...
            logging.exception("Error al cargar conceptos por defecto.")
