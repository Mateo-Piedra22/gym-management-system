...
class WhatsAppConfigWidget(QWidget):
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        # Cargar configuración editable en segundo plano para evitar bloqueos
        try:
            self.load_current_settings_async()
        except Exception as e:
            logging.debug(f"No se pudo iniciar carga asíncrona de configuración WhatsApp: {e}")
    ...
    def create_unified_config_tab(self):
        ...
        # Incluir configuración editable para asegurar que los editores existan
        try:
            edit_config_tab = self.create_config_tab()
        except Exception as e:
            logging.warning(f"No se pudo crear pestaña de configuración editable: {e}")
            edit_config_tab = None
        try:
        ...
        if edit_config_tab is not None:
            layout.addWidget(edit_config_tab)
    ...
    def load_current_settings_async(self):
        """Carga la configuración editable de WhatsApp en segundo plano con caché y timeout"""
        try:
            ttl_seconds = 120
            now_ts = datetime.now().timestamp()
            cache_ok = (
                hasattr(self, '_wa_cfg_cache_ts')
                and hasattr(self, '_wa_cfg_cache')
                and (now_ts - getattr(self, '_wa_cfg_cache_ts', 0) < ttl_seconds)
            )
            if cache_ok and isinstance(self._wa_cfg_cache, dict):
                general_flag = None
                try:
                    general_flag = self.db_manager.obtener_configuracion('enable_waitlist_prompt')
                except Exception:
                    general_flag = None
                self._apply_whatsapp_config_to_ui(self._wa_cfg_cache, general_flag)
                return

            def _load():
                cfg = self.db_manager.obtener_configuracion_whatsapp_completa()
                try:
                    gen_flag = self.db_manager.obtener_configuracion('enable_waitlist_prompt')
                except Exception:
                    gen_flag = None
                return {'cfg': cfg or self.get_default_config(), 'general_flag': gen_flag}

            def _on_success(data: dict):
                self._wa_cfg_cache = data['cfg']
                self._wa_cfg_cache_ts = datetime.now().timestamp()
                self._apply_whatsapp_config_to_ui(data['cfg'], data.get('general_flag'))

            def _on_error(msg: str):
                QMessageBox.warning(self, "Advertencia", f"No se pudo cargar configuración de WhatsApp: {msg}")
            ...
            run_in_background(
                ...
                parent=self,
                timeout_ms=6000,
                description="Cargar configuración WhatsApp",
            )
        except Exception as e:
            logging.error(f"Error al iniciar carga de configuración WhatsApp: {e}")
    ...
    def load_current_data(self):
        ...
        # Cargar plantillas en modo asíncrono para evitar bloqueo
        try:
            self.load_templates_readonly_async()
        except Exception:
    ...
    def load_templates_readonly(self):
        ...
        try:
            ...
            self._populate_templates_readonly(templates)
        except Exception as e:
            logging.error(f"Error al cargar plantillas de solo lectura: {e}")

    def load_templates_readonly_async(self):
        """Carga las plantillas en modo de solo lectura en segundo plano con timeout"""
        try:
            if not WHATSAPP_AVAILABLE:
                return

            def _load():
                return self.db_manager.obtener_plantillas_whatsapp()

            def _on_success(templates):
                self._populate_templates_readonly(templates)

            def _on_error(msg: str):
                QMessageBox.warning(self, "Advertencia", f"No se pudieron cargar plantillas: {msg}")
            ...
            run_in_background(
                ...
                parent=self,
                timeout_ms=6000,
                description="Cargar plantillas WhatsApp (solo lectura)",
            )
        except Exception as e:
            logging.error(f"Error al iniciar carga asíncrona de plantillas: {e}")

    def _populate_templates_readonly(self, templates):
        try:
            ...
            logging.error(f"Error poblando lista de plantillas: {e}")
    ...
    def get_current_config_from_ui(self) -> Dict[str, Any]:
        ...
        return {
            'phone_number': self.phone_number_edit.text().strip() if hasattr(self, 'phone_number_edit') else '',
            'webhook_url': self.webhook_url_edit.text().strip() if hasattr(self, 'webhook_url_edit') else '',
            'webhook_port': self.webhook_port_spin.value() if hasattr(self, 'webhook_port_spin') else 8000,
            'verify_token': self.verify_token_edit.text().strip() if hasattr(self, 'verify_token_edit') else '',
            'app_id': self.app_id_edit.text().strip() if hasattr(self, 'app_id_edit') else '',
            'app_secret': self.app_secret_edit.text().strip() if hasattr(self, 'app_secret_edit') else '',
            'access_token': self.access_token_edit.text().strip() if hasattr(self, 'access_token_edit') else '',
            'auto_confirm_payments': self.auto_confirm_checkbox.isChecked() if hasattr(self, 'auto_confirm_checkbox') else True,
            'auto_reminders': self.auto_reminder_checkbox.isChecked() if hasattr(self, 'auto_reminder_checkbox') else True,
            'welcome_messages': self.welcome_message_checkbox.isChecked() if hasattr(self, 'welcome_message_checkbox') else True,
            'waitlist_prompt_enabled': self.waitlist_prompt_checkbox.isChecked() if hasattr(self, 'waitlist_prompt_checkbox') else True,
            'max_messages_per_hour': self.max_messages_per_hour_spin.value() if hasattr(self, 'max_messages_per_hour_spin') else 10,
            'max_messages_per_day': self.max_messages_per_day_spin.value() if hasattr(self, 'max_messages_per_day_spin') else 50,
            'block_duration_hours': self.block_duration_spin.value() if hasattr(self, 'block_duration_spin') else 24,
            'max_retries': self.max_retries_spin.value() if hasattr(self, 'max_retries_spin') else 3,
            'retry_delay_seconds': self.retry_delay_spin.value() if hasattr(self, 'retry_delay_spin') else 300,
            'cleanup_days': self.cleanup_days_spin.value() if hasattr(self, 'cleanup_days_spin') else 30,
            'auto_cleanup': self.auto_cleanup_checkbox.isChecked() if hasattr(self, 'auto_cleanup_checkbox') else True,
            'log_level': self.log_level_combo.currentText() if hasattr(self, 'log_level_combo') else 'INFO',
            'log_to_file': self.log_to_file_checkbox.isChecked() if hasattr(self, 'log_to_file_checkbox') else True
        }
    ...
    def load_current_settings(self):
        ...
        try:
            ...
            self._apply_whatsapp_config_to_ui(cfg, None)
        except Exception as e:
            logging.error(f"Error cargando configuración en WhatsAppConfigWidget: {e}")

    def _apply_whatsapp_config_to_ui(self, cfg: Dict[str, Any], general_flag: Optional[Any]):
        try:
            # Campos básicos
            if hasattr(self, 'phone_number_edit'):
            ...
            if hasattr(self, 'cleanup_days_spin'):
                self.cleanup_days_spin.setValue(int(cfg.get('cleanup_days', 30)))
            if hasattr(self, 'auto_cleanup_checkbox'):
            ...
            if hasattr(self, 'log_to_file_checkbox'):
                self.log_to_file_checkbox.setChecked(bool(cfg.get('log_to_file', True)))
        except Exception as e:
            logging.error(f"Error aplicando configuración a la UI: {e}")
