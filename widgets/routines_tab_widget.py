...
class RoutinesTabWidget(QWidget):
    ...
    def open_user_selection_dialog(self):
         """Abre el diálogo de selección de usuarios"""
         try:
    ...
    def select_routine(self, routine_id: int):
        """Selecciona una rutina específica por ID"""
        try:
            # Primero obtener la rutina completa
            routine = self.db_manager.obtener_rutina_completa(routine_id)
            if not routine:
                print(f"Rutina con ID {routine_id} no encontrada")
                return False
            
            # Obtener el usuario asociado a la rutina
            user = self.db_manager.obtener_usuario_por_id(routine.usuario_id)
            if not user:
                print(f"Usuario con ID {routine.usuario_id} no encontrado")
                return False
            # Bloquear carga si el usuario está inactivo
            if not getattr(user, 'activo', True):
                QMessageBox.warning(self, "Usuario Inactivo", f"{user.nombre} está inactivo. No se puede cargar la rutina.")
                return False
            
            # Establecer el usuario para la rutina
            self.set_user_for_routine(user)
            
            # Buscar la rutina en el combo de rutinas del usuario
            for i in range(self.user_routines_combo.count()):
                combo_routine = self.user_routines_combo.itemData(i)
                if combo_routine and combo_routine.id == routine_id:
                    # Seleccionar la rutina en el combo
                    self.user_routines_combo.setCurrentIndex(i)
                    return True
            
            # Si no se encontró en el combo, cargar directamente
            ...
            # Mantener el valor actual de semanas o usar 4 por defecto
            ...
            print(f"Error al seleccionar rutina {routine_id}: {e}")
            return False
    ...
    def load_selected_routine_from_combo(self, index):
        ...
        if self.selected_user:
            ...
            if not getattr(self.selected_user, 'activo', True):
                ...
                return
        try:
            # Manejar tanto objetos Rutina como diccionarios
            routine_id = routine_data.get('id') if isinstance(routine_data, dict) else routine_data.id
            self.current_routine = self.db_manager.obtener_rutina_completa(routine_id)
            if self.current_routine:
                self.routine_name_input.setText(self.current_routine.nombre_rutina)
                self.routine_days_spinbox.setValue(self.current_routine.dias_semana)
                self.details_group.setVisible(True)
            ...
            QMessageBox.critical(self, "Error", f"No se pudo cargar la rutina completa: {e}")
