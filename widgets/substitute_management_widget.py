...
class SubstituteManagementWidget(QWidget):
    ...
    def load_professor_schedules(self, profesor_id: int):
        ...
        try:
            ...
            if mode == 'General':
                schedules = self.db_manager.obtener_horarios_disponibilidad_profesor(profesor_id)
                for sc in schedules:
                ...
                schedules = self.db_manager.obtener_horarios_profesor(profesor_id)
                for sc in schedules:
                    dia = sc.get('dia_semana') or sc.get('dia')
                    hi = sc.get('hora_inicio')
                    hf = sc.get('hora_fin')
    ...
    def load_substitutes(self):
        ...
        try:
            ...
            if selected_status != "Todas":
                alias = "sg" if getattr(self, 'current_mode', 'General') == "General" else "ps"
                conditions.append(f"{alias}.estado = %s")
                params.append(selected_status)
            
            # Filtro por fecha (si cambia la fecha, filtramos por ese día)
            selected_qdate = self.date_filter.date()
            if selected_qdate:
                if getattr(self, 'current_mode', 'General') == "General":
                    conditions.append("sg.fecha = %s")
                    params.append(selected_qdate.toString("yyyy-MM-dd"))
                else:
            ...
            for row, substitute in enumerate(substitutes):
                # Conversión segura a texto para evitar errores con tipos fecha/hora
                id_txt = str(substitute[0]) if substitute[0] is not None else ""
                fecha_txt = self._fmt_date(substitute[1])
                ...
                if estado_txt == "Confirmado":
                    ...
                    status_item.setBackground(QColor(255, 182, 193))  # Rosa claro
                elif estado_txt == "Pendiente":
                    status_item.setBackground(QColor(255, 255, 224))  # Amarillo claro
    ...
    def load_professors(self):
        ...
        try:
            ...
            for professor in professors:
                self.original_professor_combo.addItem(professor[1], professor[0])
                self.substitute_professor_combo.addItem(professor[1], professor[0])
    ...
    def load_substitute_info(self):
        """Carga la información detallada de la suplencia seleccionada"""
