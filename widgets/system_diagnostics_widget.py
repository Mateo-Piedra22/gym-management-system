...
class SystemDiagnosticsWidget(QWidget):
    ...
    def setup_health_overview_tab(self):
        ...
        # Panel de observabilidad de cola offline
        self.setup_offline_queue_panel(layout)
    ...
    def setup_offline_queue_panel(self, parent_layout: QVBoxLayout):
        """Crea el panel de 'Cola Offline' con gráficas de intentos y drenaje."""
        offline_group = QGroupBox("📦 Cola Offline (Observabilidad)")
        offline_layout = QVBoxLayout(offline_group)

        # Gráfica: Intentos por categoría
        self.offline_attempts_chart = MplChartWidget(figsize=(8, 4), enable_toolbar=False, enable_alerts=False)
        self.offline_attempts_chart.setMinimumHeight(300)
        offline_layout.addWidget(self.offline_attempts_chart)

        # Gráfica: Tiempo promedio de drenaje por categoría
        self.offline_drain_chart = MplChartWidget(figsize=(8, 4), enable_toolbar=False, enable_alerts=False)
        self.offline_drain_chart.setMinimumHeight(300)
        offline_layout.addWidget(self.offline_drain_chart)

        # Controles de actualización
        controls_layout = QHBoxLayout()
        refresh_btn = QPushButton("🔄 Actualizar métricas offline")
        refresh_btn.clicked.connect(self.refresh_offline_metrics)
        controls_layout.addWidget(refresh_btn)
        controls_layout.addStretch()
        offline_layout.addLayout(controls_layout)

        parent_layout.addWidget(offline_group)
        # Cargar métricas iniciales
        try:
            self.refresh_offline_metrics()
        except Exception:
            pass

    def _get_offline_observability_snapshot(self) -> dict:
        """Obtiene un snapshot con métricas de observabilidad offline.

        Intenta usar el mismo archivo de cola por defecto para evitar dependencias.
        """
        try:
            mgr = OfflineSyncManager()  # Usa ruta por defecto 'offline_queue.sqlite'
            snap = mgr.get_connectivity_snapshot()
            return snap.get('offline_observability', {}) or {}
        except Exception:
            return {}

    def refresh_offline_metrics(self):
        """Refresca gráficas de intentos y tiempo promedio de drenaje por categoría."""
        obs = self._get_offline_observability_snapshot()
        attempts = obs.get('attempts_by_category', {}) or {}
        drain = obs.get('avg_drain_sec_by_category', {}) or {}

        # Preparar datos para gráficas; asegurar categorías conocidas aunque no haya datos
        categories = sorted(set(list(attempts.keys()) + list(drain.keys())))
        if not categories:
            categories = ['db', 'whatsapp']
        attempts_values = [int(attempts.get(cat, 0)) for cat in categories]
        drain_values = [float(drain.get(cat, 0.0)) for cat in categories]

        # Plotear
        try:
            self.offline_attempts_chart.plot_bar_chart(
                x_labels=categories,
                y_values=attempts_values,
                title="Intentos completados por categoría",
                y_label="Intentos"
            )
        except Exception:
            pass
        try:
            self.offline_drain_chart.plot_bar_chart(
                x_labels=categories,
                y_values=drain_values,
                title="Tiempo promedio de drenaje (s) por categoría",
                y_label="Segundos"
            )
        except Exception:
            pass
