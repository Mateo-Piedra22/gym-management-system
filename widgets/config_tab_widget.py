...
class ConfigTabWidget(QWidget):
    ...
    def load_users_for_role_management(self):
        # Estado inicial no bloqueante y placeholder visual
        try:
            self.user_selector_combo.blockSignals(True)
            self.user_selector_combo.clear()
            self.user_selector_combo.addItem(PLACEHOLDER_LOADING_USERS, userData=None)
        finally:
            self.user_selector_combo.blockSignals(False)
            self.update_role_buttons_state()

        # Disparar carga asíncrona en hilo
        self._load_role_management_users_async()

    def _load_role_management_users_async(self):
        def _load():
            return self.db_manager.obtener_todos_usuarios()

        def _on_success(all_users):
            try:
                self.user_selector_combo.blockSignals(True)
                self.user_selector_combo.clear()
                self.user_selector_combo.addItem(PLACEHOLDER_SELECT_USER, userData=None)
                for user in all_users:
                    if getattr(user, 'rol', None) != 'dueño':
                        nombre = getattr(user, 'nombre', '') or ''
                        rol = (getattr(user, 'rol', '') or '').title()
                        self.user_selector_combo.addItem(f"{nombre} ({rol})", userData=user)
            except Exception as e:
                QMessageBox.critical(self, "Error", f"No se pudo procesar la lista de usuarios: {e}")
            finally:
                self.user_selector_combo.blockSignals(False)
                self.update_role_buttons_state()
        ...
        thread = TaskThread(_load, on_success=_on_success, on_error=_on_error)
        thread.start()
