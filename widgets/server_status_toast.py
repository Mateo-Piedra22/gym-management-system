...
class ServerStatusToast(QDialog):
    ...
    def __init__(self, parent_window, local_url: str, public_url: Optional[str] = None, message: Optional[str] = None):
    ...
    def _build_ui(self):
        ...
        urls_layout.addWidget(local_lbl)
        if self.public_url:
            ...
            urls_layout.addWidget(public_lbl)
    ...
    def _default_message(self) -> str:
        try:
            if self.public_url:
                return "Servidor local activo y túnel LocalTunnel habilitado"
            return "Servidor local activo"
        except Exception:
