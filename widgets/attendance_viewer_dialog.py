...
from PyQt6.QtCore import Qt, QDate, pyqtSignal, QTimer, QThread, pyqtSlot, QObject
...
class AttendanceViewerDialog(QDialog):
    ...
    def __init__(self, db_manager, parent=None):
        ...
        # Cargar usuarios de forma asíncrona para no bloquear la UI
        self.user_combo.clear()
        self.user_combo.addItem(PLACEHOLDER_LOADING_USERS, None)
    ...
    def connect_signals(self):
        ...
        # Señal para poblar combobox desde hilo
        self.users_loaded.connect(self._populate_users_combobox)
        
    def load_users_async(self):
        """Carga de usuarios en un hilo para evitar bloqueos de la UI (unificado con TaskThread)."""
        try:
            load_users_cached_async(
                self.db_manager,
                on_success=self.users_loaded.emit,
                on_error=lambda msg: QMessageBox.warning(self, "Error", f"Error al cargar usuarios: {msg}"),
                parent=self,
            )
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Error iniciando carga de usuarios: {str(e)}")

    @pyqtSlot(list)
    def _populate_users_combobox(self, users):
        try:
            self.user_combo.clear()
            self.user_combo.addItem(PLACEHOLDER_SELECT_USER, None)
            for user in users:
                display_name = getattr(user, 'nombre', None) or str(user)
