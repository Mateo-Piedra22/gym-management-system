...
class ClassesTabWidget(QWidget):
    ...
    def load_initial_data(self):
        self.profesores = self.db_manager.obtener_profesores(); self.load_class_types()
    def load_class_types(self):
        self.class_combo.blockSignals(True); self.class_list_widget.blockSignals(True)
        current_class_id = self.class_combo.currentData().id if self.class_combo.currentData() else None
        ...
        try:
            ...
            for c in clases:
                ...
                if c.id == current_class_id: self.class_combo.setCurrentText(c.nombre)
        except Exception as e: QMessageBox.critical(self, "Error", f"No se pudieron cargar las clases: {e}")
        finally: self.class_combo.blockSignals(False); self.class_list_widget.blockSignals(False)
    ...
    def load_schedules_for_class(self, clase_id: int):
        ...
        try:
            horarios = self.db_manager.obtener_horarios_de_clase(clase_id)
            for h in horarios:
                prof_name = h.nombre_profesor or "Sin Asignar"; item_text = f"{h.dia_semana} {h.hora_inicio}-{h.hora_fin} (Prof: {prof_name}) [{h.inscriptos}/{h.cupo_maximo}]"
                list_item = QListWidgetItem(item_text); list_item.setData(Qt.ItemDataRole.UserRole, h); self.schedule_list.addItem(list_item)
                if h.id == selected_schedule_id: self.schedule_list.setCurrentItem(list_item)
        except Exception as e: QMessageBox.critical(self, "Error", f"No se pudieron cargar los horarios: {e}")
    def load_users_for_schedule(self, horario_id: int):
        ...
        try:
            inscriptos = self.db_manager.obtener_usuarios_en_clase(horario_id)
            self.inscriptions_table.setRowCount(len(inscriptos))
            for row, inscripcion in enumerate(inscriptos):
                id_item = QTableWidgetItem(str(inscripcion['usuario_id'])); id_item.setData(Qt.ItemDataRole.UserRole, inscripcion)
                self.inscriptions_table.setItem(row, 0, id_item); self.inscriptions_table.setItem(row, 1, QTableWidgetItem(inscripcion['nombre_usuario']))
        ...
        except Exception as e: QMessageBox.critical(self, "Error", f"No se pudieron cargar los alumnos inscriptos: {e}")
    ...
    def add_user_to_class(self):
        ...
        # Obtener usuarios disponibles (excluyendo ya inscriptos y en lista de espera)
        ...
        if dialog.exec():
            ...
            if user_to_add:
                ...
                try:
                    # El método ahora retorna True si se inscribió directamente, False si se agregó a lista de espera
                    ...
                    if inscrito_directamente:
                        QMessageBox.information(self, "Inscripción Exitosa", 
                                              f"{user_to_add.nombre} ha sido inscrito en la clase.")
                    else:
                        QMessageBox.information(self, "Agregado a Lista de Espera", 
                                              f"{user_to_add.nombre} ha sido agregado a la lista de espera.\n"
    ...
    def check_and_prompt_waitlist_notification(self, horario_id: int):
        """Si hay cupo y lista de espera, pregunta si avisar al primero y envía WhatsApp."""
        try:
            import logging
            ...
            try:
                # Usar control por horario para evitar colisiones entre diferentes horarios
                now_ts = time.time()
                last_map = getattr(self, '_waitlist_prompt_last_by_schedule', {})
                last_ts = last_map.get(horario_id, 0.0)
                # Ventana de 3 segundos para prevenir prompts repetidos
            ...
            # Tomar primero por menor posición
            try:
            ...
            # Obtener datos del usuario (teléfono)
            try:
            ...
            if respuesta != QMessageBox.StandardButton.Yes:
                # Auditoría: usuario decidió no avisar
                try:
    ...
    def load_waiting_list_for_schedule(self, horario_id: int):
        """Carga la lista de espera para un horario específico"""
        self.waiting_list_table.setRowCount(0)
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"No se pudo cargar la lista de espera: {e}")
    ...
    def promote_user_from_waiting_list(self):
        ...
        # Validar estado del usuario antes de promover
        try:
            usuario = self.db_manager.obtener_usuario_por_id(espera_data['usuario_id'])
        except Exception:
            usuario = None
        if not usuario:
        ...
        if QMessageBox.question(self, "Confirmar", 
            ...
            try:
                ...
                self.db_manager.quitar_de_lista_espera(self.selected_schedule.id, espera_data['usuario_id'])
                # Inscribir en clase (debería inscribir directamente ya que verificamos cupo)
                self.db_manager.inscribir_usuario_en_clase(self.selected_schedule.id, espera_data['usuario_id'])
                
                # Actualizar ambas tablas
                self.load_users_for_schedule(self.selected_schedule.id)
                self.load_waiting_list_for_schedule(self.selected_schedule.id)
                self.load_schedules_for_class(self.selected_class_id)  # Actualizar contador
                ...
                try:
                    # Consultar configuración global para decidir si pedir confirmación de WhatsApp
                    try:
                        flag = self.db_manager.obtener_configuracion('enable_waitlist_prompt')
                        use_prompt = (str(flag).lower() == 'true') if flag is not None else ENABLE_WAITLIST_PROMPT
                    except Exception:
                    ...
                    # Registrar pero no interrumpir flujo UI
                    import logging
    ...
    def register_class_attendance(self):
        """Abre un diálogo para registrar la asistencia de los usuarios inscritos en la clase"""
        if not self.selected_schedule:
            QMessageBox.warning(self, "Sin selección", "Por favor, seleccione un horario antes de registrar asistencia.")
            return
        
        try:
            # Obtener usuarios inscritos en la clase
            inscriptos = self.db_manager.obtener_usuarios_en_clase(self.selected_schedule.id)
            
            if not inscriptos:
                QMessageBox.information(self, "Sin inscriptos", "No hay alumnos inscritos en esta clase.")
                return
            
            # Filtrar usuarios inactivos y con rol 'dueño'
            inscriptos_activos = []
            omitidos = 0
            ...
            # Crear y mostrar el diálogo de registro de asistencia
            ...
            QMessageBox.critical(self, "Error", f"No se pudo registrar la asistencia: {e}")
