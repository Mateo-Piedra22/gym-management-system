...
class ProfessorFormWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.cargar_usuarios_disponibles_async()
    ...
    def cargar_usuarios_disponibles(self):
        ...
        self.usuario_combo.addItem(PLACEHOLDER_SELECT_USER, None)
    ...
    def cargar_usuarios_disponibles_async(self):
        """Carga usuarios en hilo para no bloquear la UI."""
        try:
            ...
            def _load():
                ...
                items = []
                for u in usuarios:
                    if u.id not in usuarios_con_perfil or (
                        self.profesor_actual and u.id == self.profesor_actual.get('usuario_id')
                    ):
                        items.append((f"{u.nombre} - {u.dni}", u.id))
                return items
    ...
    def _populate_usuario_combo(self, items, error: str | None = None):
        ...
        self.usuario_combo.addItem(PLACEHOLDER_SELECT_USER, None)
        if error:
            logging.error(f"Fallo al cargar usuarios en hilo: {error}")
        for text, value in items:
            self.usuario_combo.addItem(text, value)
        # Seleccionar automáticamente el usuario del profesor actual si existe
        try:
            if self.profesor_actual and 'usuario_id' in self.profesor_actual:
                target_id = self.profesor_actual.get('usuario_id')
                for i in range(self.usuario_combo.count()):
                    if self.usuario_combo.itemData(i) == target_id:
...
class ProfessorsTabWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # Indicadores de carga y vacío para la lista de profesores
        self.professors_loading_label = QLabel("Cargando profesores...")
        self.professors_loading_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.professors_loading_label.setVisible(False)
        left_layout.addWidget(self.professors_loading_label)

        self.professors_empty_label = QLabel("Sin profesores para mostrar")
        self.professors_empty_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.professors_empty_label.setVisible(False)
        left_layout.addWidget(self.professors_empty_label)

        self.professors_progress = QProgressBar()
        self.professors_progress.setRange(0, 0)
        self.professors_progress.setTextVisible(False)
        self.professors_progress.setFixedHeight(8)
        self.professors_progress.setVisible(False)
        left_layout.addWidget(self.professors_progress)
    ...
    def cargar_profesores(self):
        """Carga la lista de profesores en la tabla sin bloquear, con caché y timeout"""
        try:
            # Mostrar indicadores de carga
            if hasattr(self, 'professors_loading_label'):
                self.professors_loading_label.setText("Cargando profesores...")
                self.professors_loading_label.setVisible(True)
            if hasattr(self, 'professors_empty_label'):
                self.professors_empty_label.setVisible(False)
            if hasattr(self, 'professors_progress'):
                self.professors_progress.setVisible(True)
            if hasattr(self, 'tabla_profesores'):
                self.tabla_profesores.setEnabled(False)

            # Usar caché si es reciente (TTL 60s)
            ttl_seconds = 60
            now_ts = time.time()
            if hasattr(self, '_profesores_cache_ts') and hasattr(self, '_profesores_cache'):
                if now_ts - getattr(self, '_profesores_cache_ts', 0) < ttl_seconds and isinstance(self._profesores_cache, list):
                    return self.actualizar_tabla_profesores(self._profesores_cache)
            ...
            def _on_success(profesores):
                self._profesores_cache = profesores
            ...
            def _on_error(msg: str):
                self.unsetCursor()
                QMessageBox.critical(self, "Error", f"No se pudo cargar la lista de profesores: {msg}")
                if hasattr(self, 'professors_loading_label'):
                    self.professors_loading_label.setVisible(False)
                if hasattr(self, 'professors_empty_label'):
                    self.professors_empty_label.setText("Error al cargar profesores")
                    self.professors_empty_label.setVisible(True)
                if hasattr(self, 'professors_progress'):
                    self.professors_progress.setVisible(False)
                if hasattr(self, 'tabla_profesores'):
    ...
    def apply_professor_filters(self, filters):
        ...
        try:
            base = getattr(self, '_profesores_cache', None)
            ...
            QMessageBox.warning(self, "Advertencia", f"No se pudieron aplicar filtros: {e}")

    def _filtrar_en_memoria(self, profesores, filters):
    ...
    def cargar_sesiones_recientes(self, profesor_id):
        ...
        try:
            # Circuit breaker: usar caché si disponible y reintentar luego
            try:
                if hasattr(self.db_manager, 'is_circuit_open') and self.db_manager.is_circuit_open():
                    sesiones_cached = None
                    try:
                        sesiones_cached = self.db_manager.cache.get('profesor_sesiones', f"recent:{profesor_id}")
                    except Exception:
                        sesiones_cached = None
                    if hasattr(self, 'lista_sesiones'):
                        self.lista_sesiones.clear()
                        if sesiones_cached:
                            for sesion_data in sesiones_cached:
                                ...
                                try:
                                    if minutos_totales:
                                        horas = int(minutos_totales // 60)
                                        minutos = int(minutos_totales % 60)
                                        horas_str = f"{horas}h {minutos}m" if horas > 0 else f"{minutos}m"
                                except Exception:
                                    pass
                            ...
                            item = QListWidgetItem("⏳ Base de datos ocupada; reintentando...")
                            self.lista_sesiones.addItem(item)
                    from PyQt6.QtCore import QTimer
                    QTimer.singleShot(1500, lambda: self.cargar_sesiones_recientes(profesor_id))
                    return
            except Exception:
                pass

            # Consulta endurecida con helper de timeouts
            with self.db_manager.get_connection_context() as conn:
                cursor = conn.cursor()
                try:
            ...
            # Cachear resultados para futuras cargas rápidas
            try:
                self.db_manager.cache.set('profesor_sesiones', f"recent:{profesor_id}", sesiones)
            except Exception:
                pass
