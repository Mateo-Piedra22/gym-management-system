...
class WhatsAppManager:
    ...
    def __init__(self, database_manager: DatabaseManager):
        """Inicializa el cliente WhatsApp con datos reales de la API"""
        ...
        # Inicializar cliente directamente con datos hardcodeados
        self._initialize_client()

    # --- Soporte mínimo para modo offline ---
    def _enqueue_offline_op(self, func_name: str, kwargs: Dict[str, Any]) -> bool:
        """Encola la operación de WhatsApp para ejecutar cuando vuelva la conexión."""
        try:
            offline_mgr = getattr(self, 'offline_sync_manager', None)
            if offline_mgr:
                offline_mgr.enqueue_whatsapp_operation(func_name, kwargs)
                logging.info(f"WhatsApp: operación {func_name} encolada para procesamiento offline")
                return True
        except Exception as e:
            logging.error(f"Error encolando operación WhatsApp offline {func_name}: {e}")
        return False
    ...
    def _initialize_client(self):
        ...
        try:
            if not self.access_token:
                print("❌ Error: Access Token no configurado")
                return False
            ...
            print(f"✅ Cliente WhatsApp inicializado correctamente")
            print(f"   Phone ID: {self.phone_number_id}")
            print(f"   WABA ID: {self.whatsapp_business_account_id}")
            print(f"   Token configurado: Sí")
            
            return True
            
        except Exception as e:
            print(f"❌ Error al inicializar cliente WhatsApp: {e}")
            return False
    
    def _setup_message_handlers(self):
        """Configura los manejadores de mensajes entrantes"""
    ...
    def enviar_mensaje_simple(self, telefono: str, mensaje: str) -> bool:
        ...
        try:
            ...
            # Enviar mensaje
            response = self.wa_client.send_message(
                to=telefono,
                text=mensaje
            )
    ...
    def enviar_confirmacion_pago(self, usuario_id: int, pago_info: Dict[str, Any], force_send: bool = False) -> bool:
        ...
        try:
            ...
            # Usar PyWa para enviar plantilla con variables
            try:
                response = self.wa_client.send_template(
                    ...
                    params=[
                        BodyText.params(
                            usuario.nombre,  # {{1}}
                            f"{pago_info.get('monto', 0):,.0f}",  # {{2}}
                            pago_info.get('fecha', datetime.now().strftime('%d/%m/%Y'))  # {{3}}
                        )
                    ]
                )
            ...
            try:
                response = self.wa_client.send_template(
                    ...
                    params=[
                        BodyText.params(
                            usuario.nombre,  # {{1}}
                            pago_actual.get('fecha_vencimiento', 'No disponible'),  # {{2}}
                            f"{pago_actual.get('monto', 0) or 0:,.0f}"  # {{3}} - Handle None values
                        )
                    ]
                )
