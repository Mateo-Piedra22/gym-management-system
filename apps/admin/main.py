...
async def crear_gimnasio(request: Request, background_tasks: BackgroundTasks, nombre: str = Form(...), subdominio: Optional[str] = Form(None), owner_phone: Optional[str] = Form(None), whatsapp_phone_id: Optional[str] = Form(None), whatsapp_access_token: Optional[str] = Form(None), whatsapp_business_account_id: Optional[str] = Form(None), whatsapp_verify_token: Optional[str] = Form(None), whatsapp_app_secret: Optional[str] = Form(None), whatsapp_nonblocking: Optional[bool] = Form(False), whatsapp_send_timeout_seconds: Optional[float] = Form(None)):
    ...
    if not sd_in:
        try:
            ...
            sd_in = ""
    if not sd_in:
        return JSONResponse({"error": "invalid_subdomain"}, status_code=400)
...
async def ver_metricas(request: Request):
    ...
    html = """
    <div class=\"dark\"><div style=\"max-width:1200px;margin:0 auto;padding:20px;font-family:system-ui\"><h1 style=\"font-size:24px;font-weight:600\">Métricas</h1><div style=\"margin:12px 0\"><a href=\"/admin/\" style=\"padding:8px 12px;border-radius:6px;background:#111;color:#fff;text-decoration:none\">Inicio</a></div><div style=\"display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px\">{cards}</div><div style=\"margin-top:16px;display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px\"><div id=\"chart-gyms-status\" style=\"height:280px\"></div><div id=\"chart-new-gyms\" style=\"height:280px\"></div><div id=\"chart-subs\" style=\"height:280px\"></div></div></div></div>
