    ...
    def _create_connection(self) -> psycopg2.extensions.connection:
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.execute("SET random_page_cost = 1.2")
...
class DatabaseManager:
    ...
    def _get_default_connection_params(self) -> dict:
        ...
        # Contraseña: primero config.json, luego variable de entorno y por último almacén seguro
        password = str(cfg.get('password', os.getenv('DB_PASSWORD', '')))
        if not password:
            try:
                import keyring
                from config import KEYRING_SERVICE_NAME, LEGACY_KEYRING_SERVICE_NAMES
                # Intentar recuperar desde la etiqueta actual
    ...
    def _crear_conexion_directa(self):
        ...
        conn = psycopg2.connect(**self.connection_params)
        conn.autocommit = False
        # Ensure timezone for direct connections
        with conn.cursor() as cursor:
            cursor.execute("SET TIME ZONE 'America/Argentina/Buenos_Aires'")
        return conn
    ...
    def obtener_todos_usuarios(self) -> List[Usuario]:
        """Obtiene todos los usuarios ordenados por rol y nombre"""
        with self.get_connection_context() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                cursor.execute("""
    ...
    def obtener_todos_pagos(self) -> List:
        ...
        try:
            with self.get_connection_context() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                    cursor.execute("SELECT * FROM pagos ORDER BY año DESC, mes DESC")
    ...
    def obtener_todas_rutinas(self) -> List:
        ...
        try:
            with self.get_connection_context() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                    cursor.execute("SELECT * FROM rutinas ORDER BY nombre")
                    return [dict(row) for row in cursor.fetchall()]
        except Exception as e:
    ...
    def obtener_usuario_por_rol(self, rol: str) -> Optional[Usuario]:
        """Obtiene el primer usuario que tenga el rol especificado"""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                with self.get_connection_context() as conn:
                    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                        cursor.execute("SELECT * FROM usuarios WHERE rol = %s AND activo = TRUE LIMIT 1", (rol,))
                        row = cursor.fetchone()
                        if row:
                            data = dict(row)
                            # Unificar nombre completo en 'nombre' si existen columnas separadas y limpiar campos no usados
                            if 'apellido' in data:
                                nombre = (data.get('nombre') or '').strip()
                                apellido = (data.get('apellido') or '').strip()
                                data['nombre'] = (f"{nombre} {apellido}".strip() if nombre or apellido else nombre or apellido)
                                data.pop('apellido', None)
                            # El sistema no maneja email en el modelo Usuario
                            data.pop('email', None)
                            # Filtrar solo los campos que existen en el dataclass Usuario
    ...
    def obtener_configuracion(self, clave: str) -> Optional[str]:
        ...
        with self.get_connection_context() as conn:
            with conn.cursor() as cursor:
                ...
                row = cursor.fetchone()
                # Manejo seguro de resultados - verificar que row existe y tiene elementos
    ...
    def obtener_tipos_cuota_activos(self) -> List[TipoCuota]:
        ...
        with self.get_connection_context() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                cursor.execute("SELECT * FROM tipos_cuota WHERE activo = TRUE ORDER BY nombre")
                return [TipoCuota(**dict(r)) for r in cursor.fetchall()]
    ...
    def crear_nota_usuario(self, nota) -> int:
        ...
        with self.get_connection_context() as conn:
            with conn.cursor() as cursor:
                ...
                RETURNING id
                """
                cursor.execute(sql, (nota.usuario_id, nota.categoria, nota.titulo, 
                                   nota.contenido, nota.importancia, nota.autor_id))
    ...
    def obtener_etiqueta_por_nombre(self, nombre: str):
        ...
        with self.get_connection_context() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                cursor.execute("SELECT * FROM etiquetas WHERE nombre = %s AND activa = TRUE", (nombre,))
                row = cursor.fetchone()
                if row:
                    return Etiqueta(
                        id=row.get('id'),
        ...
        with self.get_connection_context() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                ...
                return Etiqueta(
                    ...
                    activa=nueva_etiqueta.get('activa', True)
                )
    ...
    def obtener_etiquetas_usuario(self, usuario_id: int) -> List:
        ...
        with self.get_connection_context() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                ...
                WHERE ue.usuario_id = %s AND e.activa = TRUE
                ORDER BY e.nombre
                """
                cursor.execute(sql, (usuario_id,))
                etiquetas = []
    ...
    def obtener_asistencias_por_dia(self, dias: int = 30):
        """Devuelve lista de pares (fecha, conteo) para últimos 'dias'. Usa columnas 'fecha' y 'hora_registro'."""
        try:
            with self.get_connection_context() as conn:
                cursor = conn.cursor()
                cursor.execute(
    ...
    def obtener_asistencias_por_rango_diario(self, fecha_inicio: str, fecha_fin: str):
        """Devuelve lista de pares (fecha, conteo) para rango especificado (YYYY-MM-DD)."""
        try:
            with self.get_connection_context() as conn:
                cursor = conn.cursor()
                cursor.execute(
    ...
    def obtener_asistencias_por_hora(self, dias: int = 30):
        """Devuelve lista de pares (hora, conteo) para últimos 'dias', agrupando por EXTRACT(HOUR FROM hora_registro)."""
        try:
            with self.get_connection_context() as conn:
                cursor = conn.cursor()
                cursor.execute(
    ...
    def obtener_asistencias_por_hora_rango(self, fecha_inicio: str, fecha_fin: str):
        """Devuelve lista de pares (hora, conteo) para rango especificado, agrupado por hora de registro."""
        try:
            with self.get_connection_context() as conn:
                cursor = conn.cursor()
                cursor.execute(
    ...
    def obtener_tipos_cuota_activos(self) -> List[TipoCuota]:
        ...
        with self.get_connection_context() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM tipos_cuota WHERE activo = true ORDER BY nombre"
                )
    ...
    def obtener_asistencias_por_dia_semana(self, dias: int = 30) -> Dict[str, int]:
        """Obtiene estadísticas de asistencias por día de la semana."""
        with self.get_connection_context() as conn:
            cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    ...
    def ensure_indexes(self) -> None:
        """Crea (idempotentemente) los índices críticos si faltan y ejecuta ANALYZE."""
        try:
            with self.get_connection_context() as conn:
                with conn.cursor() as cursor:
                    # Usuarios
            ...
            logging.error(f"Error al asegurar índices: {e}")
    ...
    def obtener_tipos_cuota(self, solo_activos: bool = False) -> List[TipoCuota]:
        """Obtiene todos los tipos de cuota."""
        try:
            with self.get_connection_context() as conn:
                cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
                sql = "SELECT id, nombre, precio, icono_path, activo, fecha_creacion, descripcion, duracion_dias FROM tipos_cuota"
                if solo_activos:
                    sql += " WHERE activo = true"
                sql += " ORDER BY nombre"
                cursor.execute(sql)
                ...
                for row in cursor.fetchall():
                    tipo = TipoCuota(
                        ...
                        duracion_dias=row['duracion_dias'] if row['duracion_dias'] is not None else 30
                    )
                    tipos.append(tipo)
                return tipos
                
        except Exception as e:
            logging.error(f"Error al obtener tipos de cuota: {str(e)}")
            logging.error(f"Traceback: {e.__class__.__name__}: {e}")
            return []
    ...
    def ensure_indexes(self) -> None:
        """Crea índices opcionales para mejorar rendimiento en consultas de pagos."""
        try:
            with self.get_connection_context() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
                    ...
                    """)
                    cursor.execute("""
                        CREATE INDEX IF NOT EXISTS idx_pagos_year_month ON pagos (año, mes);
                    """)
        except Exception as e:
            logging.error(f"No se pudieron crear índices opcionales: {e}")
        ...
        try:
            ...
            with self.atomic_transaction(isolation_level="REPEATABLE READ") as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute("""
                        SELECT DISTINCT u.id, u.nombre, u.apellido, p.fecha_pago, u.tipo_cuota, tc.duracion_dias
                        FROM usuarios u
