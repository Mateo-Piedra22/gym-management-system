...
if (Test-Path $logFile) {
  ...
  if ($age -lt $thresholdMinutes) {
    ...
    try {
      $json = @{ ts = (Get-Date).ToString('s'); job = 'backup_daily'; event = 'skipped'; reason = "gate:$ageRounded" } | ConvertTo-Json -Compress
...
Add-Content -Path $logFile -Value "[INFO] Ejecutando scripts\\essential\\quick_backup_database.py con $exe..."
...
$proc = Start-Process -FilePath $exe -ArgumentList @('scripts\\essential\\quick_backup_database.py') -WorkingDirectory $repoRoot -WindowStyle Hidden -RedirectStandardOutput $tempOut -RedirectStandardError $tempErr -Wait -PassThru
...
if ($proc.ExitCode -ne 0) {
  try {
    ...
    $json = @{ ts = (Get-Date).ToString('s'); job = 'backup_daily'; event = 'error'; exitCode = $proc.ExitCode; reason = $snippet } | ConvertTo-Json -Compress
    Add-Content -Path $statusFile -Value $json
  } catch {}
} else {
  try {
    $json = @{ ts = (Get-Date).ToString('s'); job = 'backup_daily'; event = 'finished'; exitCode = $proc.ExitCode } | ConvertTo-Json -Compress
    Add-Content -Path $statusFile -Value $json
  } catch {}
}
exit $proc.ExitCode