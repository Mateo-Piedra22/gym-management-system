...
Add-Content -Path $logFile -Value "[INFO] Ejecutando scripts\\essential\\verify_system_status.py con $exe..."
...
$proc = Start-Process -FilePath $exe -ArgumentList @('scripts\\essential\\verify_system_status.py') -WorkingDirectory $repoRoot -WindowStyle Hidden -RedirectStandardOutput $tempOut -RedirectStandardError $tempErr -Wait -PassThru
...
Add-Content -Path $logFile -Value "[INFO] Finalizado scripts\\essential\\verify_system_status.py ExitCode=$($proc.ExitCode)"
