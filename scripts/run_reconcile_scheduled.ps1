# Wrapper para ejecutar reconciliación local→remoto y registrar logs
...
$logFile = Join-Path $logsDir 'reconcile_local_remote_once.log'
...
Add-Content -Path $logFile -Value "[INFO] Ejecutando scripts\\reconcile_local_remote_once.py con $exe..."

$tempOut = Join-Path $logsDir 'reconcile_local_remote_once.out.tmp'
$tempErr = Join-Path $logsDir 'reconcile_local_remote_once.err.tmp'
if (Test-Path $tempOut) { Remove-Item $tempOut -Force }
if (Test-Path $tempErr) { Remove-Item $tempErr -Force }

$proc = Start-Process -FilePath $exe -ArgumentList @('scripts\\reconcile_local_remote_once.py') -WorkingDirectory $repoRoot -WindowStyle Hidden -RedirectStandardOutput $tempOut -RedirectStandardError $tempErr -Wait -PassThru
...
Add-Content -Path $logFile -Value "[INFO] Finalizado scripts\\reconcile_local_remote_once.py ExitCode=$($proc.ExitCode)"
exit $proc.ExitCode