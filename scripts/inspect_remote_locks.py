...
def main():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT lock_action, lock_type, locking_server_id, lock_time, shared_count, shared_enable, last_lock_time, last_locking_server_id FROM public.sym_lock ORDER BY lock_action"
    )
    cols = [d.name for d in cur.description]
    rows = cur.fetchall()
    if not rows:
        print("No sym_lock entries found")
    else:
        print(" | ".join(cols))
        for r in rows:
