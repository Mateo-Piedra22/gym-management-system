...
if (Test-Path $logFile) {
  ...
  if ($age -lt $thresholdMinutes) {
    ...
    try {
      $json = @{ ts = (Get-Date).ToString('s'); job = 'outbox_flush'; event = 'skipped'; reason = "gate:$ageRounded" } | ConvertTo-Json -Compress
...
if ($proc.ExitCode -ne 0) {
  try {
    ...
    $json = @{ ts = (Get-Date).ToString('s'); job = 'outbox_flush'; event = 'error'; exitCode = $proc.ExitCode; reason = $snippet } | ConvertTo-Json -Compress
    Add-Content -Path $statusFile -Value $json
  } catch {}
} else {
  try {
    $json = @{ ts = (Get-Date).ToString('s'); job = 'outbox_flush'; event = 'finished'; exitCode = $proc.ExitCode } | ConvertTo-Json -Compress
    Add-Content -Path $statusFile -Value $json
  } catch {}
}
