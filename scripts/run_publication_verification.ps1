# Wrapper para verificar opciones de publicación remota y registrar logs
...
$logFile = Join-Path $logsDir 'publication_verify.log'
...
if (Test-Path $logFile) {
  ...
  if ($age -lt $thresholdMinutes) {
    ...
    try {
      $json = @{ ts = (Get-Date).ToString('s'); job = 'publication_verify'; event = 'skipped'; reason = "gate:$ageRounded" } | ConvertTo-Json -Compress
...
$tempOut = Join-Path $logsDir 'publication_verify.out.tmp'
$tempErr = Join-Path $logsDir 'publication_verify.err.tmp'
if (Test-Path $tempOut) { Remove-Item $tempOut -Force }
if (Test-Path $tempErr) { Remove-Item $tempErr -Force }

$proc = Start-Process -FilePath $exe -ArgumentList @('scripts\\verify_publication_options.py') -WorkingDirectory $repoRoot -WindowStyle Hidden -RedirectStandardOutput $tempOut -RedirectStandardError $tempErr -Wait -PassThru
...
if ($proc.ExitCode -ne 0) {
  try {
    ...
    $json = @{ ts = (Get-Date).ToString('s'); job = 'publication_verify'; event = 'error'; exitCode = $proc.ExitCode; reason = $snippet } | ConvertTo-Json -Compress
    Add-Content -Path $statusFile -Value $json
  } catch {}
} else {
  try {
    $json = @{ ts = (Get-Date).ToString('s'); job = 'publication_verify'; event = 'finished'; exitCode = $proc.ExitCode } | ConvertTo-Json -Compress
    Add-Content -Path $statusFile -Value $json
  } catch {}
}

Add-Content -Path $logFile -Value "[INFO] Finalizado verify_publication_options.py ExitCode=$($proc.ExitCode)"
exit $proc.ExitCode