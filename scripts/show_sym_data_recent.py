...
def main():
    with connect_local() as conn:
        cur = conn.cursor()
        cur.execute("SELECT data_id, table_name, event_type, channel_id, create_time FROM public.sym_data ORDER BY data_id DESC LIMIT 50")
        rows = cur.fetchall()
        print("Recent sym_data entries:")
        for r in rows:
